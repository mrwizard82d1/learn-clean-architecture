...
# Run the application
dotnet run --project src/ArdalisMinClean.Web

# Or run with Aspire (if using)
dotnet run --project src/ArdalisMinClean.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/ArdalisMinClean.Web -s src/ArdalisMinClean.Web
dotnet run --project src/ArdalisMinClean.Web
```
